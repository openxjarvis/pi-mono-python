"""
Emacs-style kill ring — mirrors packages/tui/src/kill-ring.ts

Supports:
- push(text, prepend, accumulate): add killed text
- peek(): most recent entry
- rotate(): cycle for yank-pop
"""
...
class KillRing:
    ...
    Consecutive kills can be accumulated into a single entry.
    Supports yank (paste most recent) and yank-pop (cycle through older entries).
    Mirrors KillRing in kill-ring.ts.
    """
